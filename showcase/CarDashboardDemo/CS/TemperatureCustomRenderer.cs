...
{
    public class TemperatureCustomRenderer : IRadialGaugeRenderer
    {
        #region Varaibles
        ...
        public void UpdateRenderer(PaintEventArgs e)
        {
            ...
            DrawRanges(e.Graphics, new System.Drawing.Drawing2D.GraphicsPath(), 180, 180, (int)DpiAware.LogicalToDeviceUnits(116), new Point(centre, centre));
            DrawTickMarks(e.Graphics, new System.Drawing.Drawing2D.GraphicsPath(), (int)DpiAware.LogicalToDeviceUnits(116), 180, 180, 4, new Point(centre, centre), (int)DpiAware.LogicalToDeviceUnits(90));
        }
        ...
        public void DrawOuterArc(Graphics e, Int32 m_GaugeArcStart, Int32 m_GaugeArcEnd, Point m_Center, int GaugeRadius)
        {
            ...
            if (GaugeRadius > 0)
            {
                e.DrawArc(new Pen(ColorTranslator.FromHtml("#a41413"), 6), new Rectangle(m_Center.X - GaugeRadius, m_Center.Y - GaugeRadius,
                   2 * GaugeRadius, 2 * GaugeRadius), m_GaugeArcStart, m_GaugeArcEnd / 2 + 23);
                e.DrawArc(new Pen(this.RadialGauge.GaugeArcColor, 6), new Rectangle(m_Center.X - GaugeRadius, m_Center.Y - GaugeRadius,
                   2 * GaugeRadius, 2 * GaugeRadius), m_GaugeArcEnd + m_GaugeArcEnd / 2 + 23, m_GaugeArcEnd / 2 - 23);
            }
        }
        ...
        public void DrawGaugeLabel(Graphics e, Point m_Center, int GaugeRadius)
        {
            ...
            e.DrawString(RadialGauge.GaugeLabel, RadialGauge.GaugeLableFont, br,
                new Point((int)((m_Center.X) - (s.Width / 2)), (int)(m_Center.Y + GaugeRadius / 9)));
            if (RadialGauge.ShowGaugeValue)
            {
                using (Font f = new Font(RadialGauge.GaugeValueFont.Name, RadialGauge.GaugeValueFont.Size + 4))
                {
                    ...
                    s = e.MeasureString("00", RadialGauge.GaugeValueFont);
                    Rectangle rect = new Rectangle((new Point((int)((m_Center.X) - (s.Width - 5)), (int)(m_Center.Y - GaugeRadius / 4 + 1))), new Size((int)s.Width + 30, (int)s.Height + 1));
                    e.DrawRectangle(new Pen(ColorTranslator.FromHtml("#3d3939")), new Rectangle((new Point((int)((m_Center.X) - (s.Width - 5)), (int)(m_Center.Y - GaugeRadius / 4 + 1))), new Size((int)s.Width + 30, (int)s.Height + 1)));
                    e.DrawLine(new Pen(Color.Black), rect.Left, rect.Bottom, rect.Right, rect.Bottom);
                    e.DrawString(RadialGauge.Value.ToString(), f, br,
                   new Point((int)((m_Center.X) - (s.Width / 2)), (int)(m_Center.Y - GaugeRadius / 4)));
                }
            }
        ...
        public void DrawTickMarks(Graphics graphics, GraphicsPath gp, Int32 m_GaugeArcRadius, Int32 m_GaugeArcStart, Int32 m_GaugeArcEnd, Int32 m_MajorTickMarkWidth, Point m_Center, int m_ScaleNumbersRadius)
        {
            ...
            while (countValue <= (RadialGauge.MaximumValue - RadialGauge.MinimumValue))
            {
                ...
                if (countValue > 50)
                {
                    ...
                    graphics.DrawLine(new Pen(ColorTranslator.FromHtml("#a41413"), m_MajorTickMarkWidth),
                                  (Single)(m_Center.X),
                                  (Single)(m_Center.Y),
                                  (Single)(m_Center.X + 2 * lineminervalue * Math.Cos((m_GaugeArcStart + countValue * m_GaugeArcEnd / (RadialGauge.MaximumValue - RadialGauge.MinimumValue)) * Math.PI / 180.0)),
                                  (Single)(m_Center.Y + 2 * lineminervalue * Math.Sin((m_GaugeArcStart + countValue * m_GaugeArcEnd / (RadialGauge.MaximumValue - RadialGauge.MinimumValue)) * Math.PI / 180.0)));
